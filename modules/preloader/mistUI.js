...
// also certainly remove the web3.sero.ase one
...
ipcRenderer.on('uiAction_runTests', (e, type) => {
    if (type === 'webview') {
        web3.sero.getAccounts((error, accounts) => {
            if (error) return;

            web3.sero.getCoinbase((coinbaseError, coinbase) => {
                ...
                Tracker.autorun(() => {
                    const accountList = _.pluck(SeroAccounts.find({}, { fields: { address: 1 } }).fetch(), 'address');
