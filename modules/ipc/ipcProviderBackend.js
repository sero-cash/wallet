...
/**
The IPC provider backend filter and tunnel all incoming request to the sero node.
...
const seroNode = require('../seroNode');
...
class IpcProviderBackend {
    constructor() {
        ...
        seroNode.on('state', _.bind(this._onNodeStateChanged, this));
            ...
            if (!socket.isConnected) {
                ...
                if (!this._connectionPromise[ownerId]) {
                    this._connectionPromise[ownerId] = Q.try(() => {
                        ...
                        if (seroNode.state !== seroNode.STATES.CONNECTED) {
                            return new Q((resolve, reject) => {
                                const onStateChange = (newState) => {
                                    if (seroNode.STATES.CONNECTED === newState) {
                                        seroNode.removeListener('state', onStateChange);

                                        log.debug(`SERO node connected, resume connecting socket ${ownerId}`);

                                        resolve();
                                    }
                                };

                                seroNode.on('state', onStateChange);
                            });
                        }
                    })
                    .then(() => {
    ...
    /**
     * Handler for when SERO node state changes.
     *
     * Auto-reconnect sockets when sero node state changes
     *
    ...
    _onNodeStateChanged(state) {
        ...
        case seroNode.STATES.STOPPING:
            log.info('SERO node stopping, disconnecting sockets');
    ...
    _sendRequest(isSync, event, payload) {
        ...
        .then((returnValue) => {
            ...
            // console.log('Return', ownerId, returnValue);
