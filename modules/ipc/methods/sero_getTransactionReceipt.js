...
/**
 * Process method: sero_getTransactionReceipt
 */
...
module.exports = class extends BaseProcessor {
    ...
    async exec(conn, payload) {
        ...
        // Sends regular sero_getTransactionReceipt request
        const ret = await conn.socket.send(payload, {
            fullResult: true
        });

        // If that contains a contractAddress already, fine.
        if (ret && ret.result &&  ret.result.result && ret.result.result.contractAddress != null) {
            return ret.result;
        }
        ...
        const transactionInfo = await conn.socket.send({
            jsonrpc: '2.0',
            id: _.uuid(),
            method: 'sero_getTransactionByHash',
            params: [txHash]
        }, { fullResult: true });
