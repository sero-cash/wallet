    ...
    'items': function(){
        ...
        if(searchQuery) {
            ...
            items = _.filter(items, function(item){
                ...
                // search value
                if(pattern.test(SeroTools.formatBalance(item.value, '0,0.00[000000] unit')))
                    return item;
...
Template['elements_transactions_row'].helpers({
    ...
    */
    'incomingTx': function(account){
        var account = SeroAccounts.findOne({_id: account}) || Wallets.findOne({_id: account});
        return !!((account && this.from !== account.address) ||
                  (!account && (SeroAccounts.findOne({address: this.to}) || Wallets.findOne({address: this.to}))));
    },
    /**
    ...
    */
    'unConfirmed': function() {
        if(!this.blockNumber || !SeroBlocks.latest.number)
            return {
                confirmations: 0,
                percent: 0
            };

        var currentBlockNumber = SeroBlocks.latest.number + 1,
