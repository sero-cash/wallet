...
Template['elements_executeContract'].onCreated(function(){
    ...
    web3.sero.getCode(template.data.address, function(e, code) {
        if(!e && code.length > 2) {
...
Template['elements_executeContract'].helpers({
    ...
    */
    'reactiveContext': function() {
        var contractInstance = web3.sero.contract(this.jsonInterface).at(this.address);
...
Template['elements_executeContract'].events({
    ...
    */
    'change .select-contract-function': function(e, template){
        localStorage.setItem('executeData', null);
...
Template['elements_executeContract_constant'].onCreated(function(){
    ...
    this.autorun(function() {
        ...
        if (args){
            for (v in args){
                if( typeof args[v] !=='function' && args[v] === ''){
                    return;
                }
            }
        }
...
Template['elements_executeContract_function'].events({
    ...
    */
    'change .abi-input, input .abi-input, blur .abi-input': function(e, template) {
        ...
        inputs.push(cb);
        template.data.contractInstance[template.data.name].getData.apply(null, inputs);
    ...
    */
    'click .execute': function(e, template){
        var to = template.data.contractInstance.address,
            ...
            // estimatedGas = localStorage.getItem('estimatedGas')
            ...
            data = localStorage.getItem('executeData');
        ...
        if(selectedAccount) {
            ...
            var sendTransaction = function(estimatedGas){
                ...
                if(contracts['ct_'+ selectedAccount._id]) {
                    ...
                    var accounts = SeroAccounts.find({name: {$exists: true}}, {sort: {name: 1}}).fetch();
                ...
                // SIMPLE TX
                    ...
                    // if (template.data.contractInstance.abi){
                    //     params['abi']= template.data.contractInstance.abi
                    // }
                    // if (template.data.inputs){
                    //     var pairs = [];
                    //     for(v in template.data.inputs){
                    //         var p = {};
                    //         p[template.data.inputs[v].type]=template.data.inputs[v].value;
