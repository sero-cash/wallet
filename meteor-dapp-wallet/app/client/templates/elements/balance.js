...
Template['elements_balance'].helpers({
    ...
    */
    'convertedBalance': function(){
        ...
        if (SeroTools.getUnit() === 'nosero') return 'ta';

        if(balance){
            if(SeroTools.getUnit() === 'sero')
                return SeroTools.formatBalance(TemplateVar.get('balance'), (this.showAllDecimals? '0,0.00[00000000000000]' : '0,0.00') );
            else if(SeroTools.getUnit() === 'ta')
                return SeroTools.formatBalance(TemplateVar.get('balance'), (this.showAllDecimals? '0,0.00[00000000000000]' : '0,0.00'));
            else
                return SeroTools.formatBalance(TemplateVar.get('balance'), '0,0.00[000000]');
        }
    },
    /**
