...
addTransaction = function(log, from, to, value){
    ...
    var block = web3.sero.getBlock(log.blockNumber, false, function(err, block){
        if(!err) {

            web3.sero.getTransaction(log.transactionHash, function(err, transaction) {
                if(!err && transaction) {
                    web3.sero.getTransactionReceipt(log.transactionHash, function(err, receipt){
...
var updateTransaction = function(newDocument, transaction, receipt){
    ...
    if(receipt && transaction) {
        ...
        if(!newDocument.contractAddress && receipt.contractAddress) {
            web3.sero.getCode(receipt.contractAddress, function(e, code) {
                if(!e && code.length > 2) {
    ...
    if(newDocument.outOfGas) {
        ...
        if(SeroAccounts.findOne({address: newDocument.from})) {
            web3.sero.getBalance(newDocument.from, newDocument.blockNumber, function(e, now){

                if (now === {}) {
                    console.log(newDocument.address+' balance is nil ')
                    return
                }
                console.log('now.sero:'+now.sero)

                if(!e) {
                    web3.sero.getBalance(newDocument.from, newDocument.blockNumber-1, function(e, then){
                        if (then === {}) {
                            console.log(newDocument.address+' balance is nil ')
                            return
                        }
                        console.log('then.sero:'+then.sero)

                        if(!e && now.sero.toString(10) !== then.sero.toString(10)) {
    ...
    var checkTransactionConfirmations = function(tx){
        ...
        if(!tx.confirmed && tx.transactionHash) {
            ...
            var updateTransactions = function(e, blockHash){
                ...
                if(!e) {
                    var confirmations = (tx.blockNumber && SeroBlocks.latest.number) ? (SeroBlocks.latest.number + 1) - tx.blockNumber : 0;
                    confCount++;
                    ...
                    if(confirmations < ethereumConfig.requiredConfirmations && confirmations >= 0) {
                        ...
                        web3.sero.getTransaction(tx.transactionHash, function(e, transaction){
                            web3.sero.getTransactionReceipt(tx.transactionHash, function(e, receipt){
                            ...
                            web3.sero.getTransactionReceipt(tx.transactionHash, function(e, receipt){
                                if(!e) {
                                    ...
                                    if(!transaction || !transaction.blockNumber) {
                                        ...
                                        web3.sero.getBlock(transaction.blockNumber, function(e, block) {
                                            if(!e) {
            ...
            var filter = web3.sero.filter('latest').watch(function(e, blockHash) {
    ...
    collectionObservers[collectionObservers.length] = Transactions.find({}).observe({
        ...
        */
        added: function(newDocument) {
            var confirmations = SeroBlocks.latest.number - newDocument.blockNumber;
        ...
        */
        removed: function(document) {
            ...
            SeroAccounts.update({address: document.from}, {$pull: {
                transactions: document._id
            }});
            SeroAccounts.update({address: document.to}, {$pull: {
                transactions: document._id
            }});
        }
    });
