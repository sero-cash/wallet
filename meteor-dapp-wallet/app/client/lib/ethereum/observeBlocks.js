...
updateBalances = function() {
    ...
    _.each(walletsAndContracts, function(account){
        ...
        if(account.address) {
            web3.sero.getBalance(account.address, function(err, res){
                ...
                if(!err && typeof res.sero !== 'undefined') {
                    // is of type wallet
                    if(account.creationBlock) {
                        Wallets.update(account._id, {$set: {
                            balance: res.sero.toString(10)
                        }});
                    } else {
    ...
    _.each(creatingWallets, function(wallet){
      ...
      web3.sero.getTransactionReceipt(wallet.transactionHash, function(error, receipt) {
    ...
    // // UPDATE ENS
    // var allAccounts = SeroAccounts.find().fetch().concat(walletsAndContracts);
    ...
    //
    //             if (!err && account.address == returnedAddr){
    //                 //if (!err && account.address.toLowerCase() == returnedAddr){
    //                 SeroAccounts.update({address: account.address}, {$set:{ name: name, ens: true, ensCheck: now}});
    ...
    //             } else {
    //                 SeroAccounts.update({address: account.address}, {$set:{ens: false, ensCheck: now}});
    ...
    var walletsContractsAndAccounts = SeroAccounts.find().fetch().concat(Wallets.find().fetch());
