...
web3.sero.isSyncing(function(error, syncing) {
    if(!error) {
...
var connect = function(){
    if(web3.isConnected()) {
        ...
        // only start app operation, when the node is not syncing (or the sero_syncing property doesn't exists)
        web3.sero.getSyncing(function(e, sync) {
            if(e || !sync) {
                connectToNode();
            } else {
                SeroAccounts.init();
        ...
        Meteor.setTimeout(function(){
            ...
            var geroRPC = (web3.admin) ? 'gero' : 'gero --rpc --rpccorsdomain "'+window.location.protocol + '//' + window.location.host+'"';
            console.log('geroRPC:::',geroRPC)
