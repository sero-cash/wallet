...
@method (isSeroUnit)
**/
Template.registerHelper('isSeroUnit', function(){
...
Template.registerHelper('selectAccounts', function(hideWallets){
    var accounts = SeroAccounts.find({balance:{$ne:"0"}}, {sort: {balance: 1}}).fetch();

    if(hideWallets !== true)
        accounts = _.union(Wallets.find({owners: {$in: _.pluck(SeroAccounts.find().fetch(), 'address')}, address: {$exists: true}}, {sort: {name: 1}}).fetch(), accounts);
...
Formats a given transactions balance
    ...
    {{formatTransactionBalance value exchangeRates "sero"}}
