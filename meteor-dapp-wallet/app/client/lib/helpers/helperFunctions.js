...
Helpers.getOwnedAccountFrom = function(accountList){
    ...
    var accounts = SeroAccounts.find({}, {sort: {balance: 1}}).fetch();
...
Helpers.formatNumberByDecimals = function(number, decimals){
    ...
    return SeroTools.formatNumber(new BigNumber(number, 10).dividedBy(Math.pow(10, decimals)), numberFormat);
};
...
Helpers.checkChain = function(callback){
    ...
    web3.sero.getCode(originalContractAddress, function(e, code){
...
Helpers.isWatchOnly = function(id) {
    return !Wallets.findOne({_id: id, owners: {$in: _.pluck(SeroAccounts.find({}).fetch(), 'address')}});
};
...
/**
Gets the docuement matching the given addess from the SeroAccounts or Wallets collection.
...
Helpers.getAccountByAddress = function(address, reactive) {
    ...
    if(_.isString(address))
        ...
        address = address;
    return SeroAccounts.findOne({address: address}, options) || Wallets.findOne({address: address}, options) || CustomContracts.findOne({address: address}, options);
};

/**
Gets the docuement matching the given query from the SeroAccounts or Wallets collection.
...
Helpers.getAccounts = function(query, reactive) {
    ...
    return SeroAccounts.find(query, options).fetch().concat(Wallets.find(query, options).fetch());
};

/**
Gets the docuement matching the given addess from the SeroAccounts or Wallets collection and returns its name or address.
...
Helpers.formatTransactionBalance = function(value, exchangeRates, unit) {
    ...
    return SeroTools.formatBalance(value.sero, format + '[00000000000000] UNIT');
};
...
Helpers.getENSName = function(address, callback) {
    ...
    var node = namehash(address.replace('0x','')+'.addr.reverse');
    var ensContract = web3.sero.contract(ensContractAbi);
    var resolverContract = web3.sero.contract(resolverContractAbi);
