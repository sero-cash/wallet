updateMistBadge = function(){
    ...
    if(typeof mist !== 'undefined' && (!conf || !conf.confirmedOwners.length)) {
        var accounts = SeroAccounts.find({}).fetch();
        ...
        mist.menu.setBadge(SeroTools.formatBalance(balance, '0.0 a','sero') + ' SERO');
    }
};
...
updateMistMenu = function(){
    ...
    var accounts = _.union(Wallets.find({}, {sort: {name: 1}}).fetch(), SeroAccounts.find({}, {sort: {name: 1}}).fetch());
    ...
    Meteor.setTimeout(function(){
        ...
        _.each(accounts, function(account, index){
            mist.menu.add(account._id,{
                position: 3 + index,
                name: account.name,
                badge: SeroTools.formatBalance(account.balance, "0 a", 'sero')+ ' SERO',
