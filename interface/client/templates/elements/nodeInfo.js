...
var getMining = function(template) {
    web3.sero.getMining(function(e, res) {
        if(!e && res) {
            web3.sero.getHashrate(function(e, res) {
                if(!e) {
...
Template['elements_nodeInfo'].onCreated(function(){
    ...
    // CHECK FOR NETWORK
    web3.sero.getBlock(0, function(e, res){
        if(!e){
    ...
    // CHECK SYNCING
    this.syncFilter = web3.sero.isSyncing(function(error, syncing) {
        if(!error) {
...
Template['elements_nodeInfo'].helpers({
    ...
    */
    formattedBlockNumber: function () {
        return numeral(SeroBlocks.latest.number).format('0,0');
    },
    /**
    ...
    */
    timeSinceBlock: function () {
        var timeSince = moment(SeroBlocks.latest.timestamp, 'X');
        var now = moment();
        var diff = now.diff(timeSince, 'seconds');

        if (!SeroBlocks.latest.timestamp) {
            return '-';
        }
